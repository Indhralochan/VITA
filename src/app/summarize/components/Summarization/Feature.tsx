...
const Feature = ({ url }) => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`http://localhost:4000/summarize?url=${url}`);
        ...
        <>
            ...
            <div className="w-full rounded-xl bg-gray-600/25 p-2 ring-1 ring-inset ring-gray-700/10  lg:rounded-2xl lg:p-4 justify-center mt-10" style={{ minHeight: "300px" }}>
                {loading ? "Loading..." : (
                    data ? (
                        <>
                            <h1>{data.data[0].summary_text}</h1>
                        </>
                    ) : "Data not available"
                )}
            </div>
            <div className="mx-auto justify-center ">
                <Button variant="secondary" className='text-center py-5 mt-10' onClick={()=>{reFetchData}}>
                    Re-Summarize
                </Button>
            </div>
        </>
